  ...
  return (
    <div 
      className={`group relative min-h-[24px] flex items-start rounded transition-all duration-150 ease-in-out
        ${isSelected ? 'bg-primary/10 ring-1 ring-primary/30 shadow-sm' : 'hover:bg-muted'}`}
        ...
        {showCommands && (
          ...
          >
            {filteredCommands.map((cmd, index) => (
              ...
              >
                <span className="flex-shrink-0 w-6 h-6 flex items-center justify-center bg-accent rounded text-muted-foreground">
                  {cmd.icon}
                </span>
